...
const MultipleYearSDropdown = ({ years, selectedYears, setSelectedYears }) => {
  ...
  useEffect(() => {
    if (open) {
      const selectedElements =
        dropdownRef.current.querySelectorAll(".wrapper.selected");
      if (selectedElements.length > 0) {
        selectedElements[selectedElements.length - 1].scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }
  ...
  const handleYearClick = (year) => {
    setSelectedYears((prevYears) => {
      const yearValue = +year.replace("y_", "");
      const isSelected = prevYears.includes(yearValue);

      if (isSelected) {
        // Remove year if it's selected, but not if it's the last one
        if (prevYears.length > 1) {
          return prevYears.filter((y) => y !== yearValue);
        } else {
          // If only one year is left, don't remove it
          return prevYears;
        }
      } else {
        // Add year if it's not selected
        if (prevYears.length < 3) {
          return [...prevYears, yearValue];
        } else {
          // If 3 years are already selected, replace the oldest one
          const newYears = [...prevYears];
          newYears.shift(); // Remove the first (oldest) year
          return [...newYears, yearValue]; // Add the new year
        }
      }
    });
  };
          ...
          {years
            ...
            .map(
              (
                ...
                <div
                  className={`wrapper ${
                    selectedYears.includes(+year.replace("y_", ""))
                      ? "selected"
                      : ""
                  }`}
                  key={i}
...
export default MultipleYearSDropdown;
