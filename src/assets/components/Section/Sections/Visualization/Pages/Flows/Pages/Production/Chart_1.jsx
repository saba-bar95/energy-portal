...
const Chart_1 = () => {
  ...
  const text = {
    ka: {
      title: "ენერგორესურსების წარმოება",
      unit: "ათასი ტნე",
    },
    en: {
  ...
  const colorMap = useMemo(
    () => ({
      ელექტროენერგია: "#6FAEA9",
      "ბიოსაწვავი და ნარჩენები": "#556EB0",
      ქვანახშირი: "#5A9FDE",
      "გეოთერმული, მზის და ა.შ.": "#D5A43F",
      "ბუნებრივი გაზი": "#BE6433",
      ნავთობპროდუქტები: "#994C8E",
    }),
    []
  );
    ...
    const fetchData = async () => {
      try {
        ...
        const uniqueCategories = filteredData.map((item) => ({
          key_ge: item.name_ge,
          key_en: item.name_en,
          color: colorMap[item.name_ge] || "#6FAEA9", // Fallback color if not found
        }));
