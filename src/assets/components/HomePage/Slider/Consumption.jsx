...
const EnergyProduction = () => {
  ...
  const [fullConsumption, setFullConsumption] = useState(null);

  const fullConsumptionID = 9;
  const fullConsumptionName = 1;
  const fullConsumptionCode = 42;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const rawData = await fetchDataWithCodes(fullConsumptionID);

        const filteredData = rawData.filter(
          (el) =>
            el.name === fullConsumptionName &&
            el.legend_code === fullConsumptionCode
        );
        setFullConsumption(filteredData[0][`y_${lastYear}`]);
      } catch (error) {
        console.log("Fetch error:", error);
      }
    };
        ...
        return (
          <div className="widget-wrapper" key={index}>
            ...
            <div className="middle">
              <h2>{((lastYearValue / fullConsumption) * 100).toFixed(1)}</h2>
